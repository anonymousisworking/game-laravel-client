...
class UserController extends Controller
{
	...
	public function changeLocation($locationId)
	{
		...
		$locations = Location::whereIn('id', [Auth::user()->loc, $locationId])->get();
		
		if (count($locations) !== 2) return;

		foreach ($locations as $location) {
			foreach (['loc_coords', 'loc_access'] as $key) {
				$location->{$key} = json_decode($location->{$key});
			}

			if ($location->id == Auth::user()->loc && array_search($locationId, $location->loc_access) === false) return;
		}

		User::where('id', Auth::user()->id)->update(['loc' => $locationId]);
